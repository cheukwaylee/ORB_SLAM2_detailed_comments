...
cv::Mat computeMtcwUseLK(KeyFrame *lastKeyFrame,
                         cv::Mat color, // [in] current frame in RGB
                         bool lastColorIsKeyFrame,
                         cv::Mat K, cv::Mat mDistCoef,
                         cv::Mat &mTcw,         // [out] current frame pose
                         int &mnMatchesInliers) // [out] the number of current frame Matches Inliers
{
    int obsPlus = 0;
    ...
    // 上一帧是关键帧的情况
    // use map points in last keyframe as current frame's map point
    if (lastColorIsKeyFrame || keypoints.empty())
    {
        ...
        {
            //  map point existing && its observation time not so few
            if (lastKeyFrame->mvpMapPoints[i] && lastKeyFrame->mvpMapPoints[i]->Observations() > obsPlus) // TODO if the program died here, try to change 1 to 0
            {
    ...
    //? TODO CV_BGR2GRAY ??? bug? not RGB?
    cvtColor(last_color, last_gray, CV_BGR2GRAY);
    cvtColor(color, gray, CV_BGR2GRAY);
    //    cout<<"preKeyPointNum"<<prev_keypoints.size()<<endl;
    //计算光流
    cv::calcOpticalFlowPyrLK(last_gray, gray,
                             prev_keypoints, // [in]
                             next_keypoints, // [in as init & out]
                             status, error);

    /** 把跟丢的点删掉**/
    int i = 0;
    // keypoints = next_keypoints, LK tracking successful result
    for (auto iter = keypoints.begin(); iter != keypoints.end(); i++)
    {
        ...
        // edit keypoints' coordinate
        *iter = next_keypoints[i];
        iter++;
    }

    // i, here, is equal to keypoints.size()

    //    cout<<"tracked keypoints: "<<keypoints.size()<<endl;
    i = 0;
    // erase the match mappoint while the keypoint is erased
    ...
    // all this three are required NON-empty
    if (!(mappointInCurrentFrame.empty() || keypoints.empty() || mDistCoef.empty()))
    {
        ...
        cv::solvePnPRansac(mappointInCurrentFrame,
                           keypoints,
                           K, mDistCoef,
                           R_vector, T, // [out] pose estimation
                           false, 50, 3, 0.98, ransacInlier, cv::SOLVEPNP_ITERATIVE);
        ...
        mTcw = Rt_float; // current frame pose [out to Tracking.cc]
    }
