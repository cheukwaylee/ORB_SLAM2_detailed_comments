...
namespace ORB_SLAM2
{
    ...
    public:
        ...
        // ConsistentGroup.first对应每个“连续组”中的关键帧集合， // 组内元素的集合,代表了这个组
        // ConsistentGroup.second为每个“连续组”的序号 // 这个组的连续的链的长度
    ...
    public:
         ...
         * @param[in] pDB     词袋数据库
         * @param[in] pVoc    词典
         * @param[in] bFixScale  表示sim3中的尺度是否要计算,对于双目和RGBD情况尺度是固定的,s=1,bFixScale=true;
    ...
    protected:
         ...
         * 2. 将闭环帧以及闭环帧相连的关键帧的MapPoints 和 与当前帧相连的关键帧的点进行匹配（相连关键帧+当前帧---闭环帧+相连关键帧）     \n
        ...
        /// 前一关键帧的闭环候选关键帧组 //上一次执行的时候产生的连续组s
        std::vector<ConsistentGroup> mvConsistentGroups;

        /// 从上面的关键帧中进行筛选之后得到的具有足够的"连续性"的关键帧
        //  -- 这个其实也是相当于更高层级的、更加优质的闭环候选帧 // 最终筛选后得到的闭环帧
        std::vector<KeyFrame *> mvpEnoughConsistentCandidates;
        /// 和当前关键帧相连的关键帧形成的"当前关键帧组" :当前关键帧及其共视关键帧
        std::vector<KeyFrame *> mvpCurrentConnectedKFs;

        /// 下面的变量中存储的地图点
        // 在"当前关键帧"中成功地找到了匹配点的地图点的集合 // "当前关键帧"和[闭环关键帧组的所有地图点]进行投影得到的匹配点
        std::vector<MapPoint *> mvpCurrentMatchedPoints;
        /// 闭环关键帧[mpMatchedKF]上的所有相连关键帧+自身 的地图点
        std::vector<MapPoint *> mvpLoopMapPoints;

        // 当得到了当前关键帧的闭环关键帧以后,计算出来的从[世界坐标系w]到[当前帧c]的sim3变换
        g2o::Sim3 mg2oScw;
        // [mg2oScw]的cv::Mat格式版本
        cv::Mat mScw;
        ...
        /// 全局BA线程是否在进行
        bool mbRunningGBA;
        /// 全局BA线程在收到停止请求之后 是否停止的标志 //? 可是直接使用上面变量的逆不就可以表示了吗? //? 表示全局BA工作是否正常结束?
        bool mbFinishedGBA;
