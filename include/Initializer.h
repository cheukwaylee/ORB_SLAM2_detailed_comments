...
namespace ORB_SLAM2
{
    ...
    private:
        ...
        /**
         * @brief  用DLT方法求解单应矩阵Homography
         * @details 这里最少用4对点就能够求出来，不过这里为了统一还是使用了8对点求最小二乘解
         *
        ...
        /**
         * @brief  根据特征点匹配求 Fundamental matrix（normalized 8点法）
         * @details 注意F矩阵有秩为2的约束，所以需要两次SVD分解
        ...
        /**
         * @brief 对给定的 Homography matrix打分,需要使用到卡方检验的知识
         *
         * @param[in] H21            从参考帧(1)到当前帧(2)的单应矩阵
         * @param[in] H12            从当前帧(2)到参考帧(1)的单应矩阵
         ...
         * @param[in] minParallax        对特征点的三角化测量中，认为其测量有效时需要满足的最小视差角
         *                        （如果视差角过小则会引起非常大的观测误差）,单位是角度
        ...
        /**
         * @brief 给定 投影矩阵P1,P2 和 图像上的匹配特征点kp1,kp2 ，从而计算三维点坐标
         *
        ...
        /**
         * @brief  进行 Cheirality Check （手性） ，从而进一步找出F分解后最合适的解
         * @details ReconstructF调用该函数进行cheirality check，从而进一步找出F分解后最合适的解(4选1)，保证地图点的深度为正
         *
        ...
        /**
         * @brief  分解Essential矩阵
         * @details F矩阵通过结合内参可以得到Essential矩阵，分解E矩阵将得到4组解 \n
