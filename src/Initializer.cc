...
namespace ORB_SLAM2
{
    ...
    /**
     * @brief 计算 F基础矩阵 和 H单应性矩阵 ，
     *     选取最佳的来恢复出最开始两帧之间的相对姿态，并进行三角化得到初始地图点
     *
     * @note  这不是构造函数！！
     *
     * Step 1 重新记录特征点对的匹配关系
     * Step 2 在所有匹配特征点对中随机选择8对匹配特征点为一组，用于估计 H矩阵 和 F矩阵
     * Step 3 计算 Fundamental矩阵 和 Homography矩阵 ，为了加速分别开了线程计算
     ...
     *
     * @param[in & out] R21       相机从参考帧(1)到当前帧(2)的旋转
     * @param[in & out] t21       相机从参考帧(1)到当前帧(2)的平移
                    ...
                    vector<bool> &vbTriangulated)
    {
        ...
        //获取当前帧的去畸变之后的特征点 //保存在initializer的成员变量里面
        ...
        mvbMatched1.resize(mvKeys1.size()); //? resize之后用.empty()返回true

        // Step 1 重新记录特征点对的匹配关系存储在 mvMatches12 ，是否有匹配存储在 mvbMatched1
        // 将 vMatches12 （输入参数，有冗余） 转化为 mvMatches12（只记录了匹配关系）
        ...
        mvSets = vector<vector<size_t>>(
            ...
            vector<size_t>(8, 0) //这个则是第二维元素的初始值，也就是第一维。
                                 // 这里其实也是一个第一维的构造函数，第一维vector有8项，每项的初始值为0.
        );
    ...
    void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
    {
        ...
        for (int it = 0; it < mMaxIterations; it++)
        {
            ...
            // 解得： vPn2i = Hn * vPn1i
            // 特征点归一化：vPn1 = T1 * mvKeys1
            //        vPn2 = T2 * mvKeys2
            // 得到：    T2 * mvKeys2 =  Hn * T1 * mvKeys1
            // 进一步得到： mvKeys2  = T2.inv * Hn * T1 * mvKeys1

            // 去归一化（尺度恢复）：未归一化的关键点对 之间的变换
            // mvKeys2  = H21i * mvKeys1
            H21i = T2inv * Hn * T1;
            //然后计算逆
            H12i = H21i.inv();
            ...
            currentScore = CheckHomography(
                ...
                mSigma            // 输入方差，默认为1 // TODO  测量误差，在Initializer类对象构造的时候，由外部给定的
            );
    ...
    void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
    {
        ...
        for (int it = 0; it < mMaxIterations; it++)
        {
            ...
            // Step 3 八点法计算基础矩阵
            // 没有归一化：x2^t * F * x1 = 0
            // 但是这里的 vPn1i vPn2i 经过归一化，所以得到的Fn也是归一化的
            ...
            // 去归一化
            // 从   (mvKeys2^t * T2^t) * Fn * (T1 * mvKeys1) = 0
            // 恢复到 x2^t * F21 * x1 = 0
            // 所以  F21 = T2^t) * Fn * (T1
            F21i = T2t * Fn * T1;
    ...
    /**
     * @brief 用DLT方法求解单应矩阵H vP2 = H21 * vP1
     * @note  这里最少用4对点就能够求出来，不过这里为了统一还是使用了8对点求最小二乘解
     *
        ...
        const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
    {
        ...
        // 使用DLT (direct linear transform) 求解该模型
        // x' = a H x
        // 左右同时左乘 (x')的斜对称矩阵 (x')^
        // ---> 0  (因为方向相同) = (x') 叉乘 (H x)
        // (取前两行就可以推导出下面的了)
        ...
        // 构造用于计算的矩阵 A
        cv::Mat A(
            2 * N, //行，注意每一个点对的数据对应两行
            9,     //列
            CV_32F // float数据类型
        );

        // 构造矩阵A，将每个特征点对添加到矩阵A中的元素
        for (int i = 0; i < N; i++)
        {
            //获取特征点对的像素坐标
        ...
        cv::SVDecomp(
            ...
            u,                   //输出，矩阵U
            vt,                  //输出，矩阵V^T
            cv::SVD::MODIFY_A |  //输入，MODIFY_A是指允许计算函数可以修改待分解的矩阵，
                                 // 官方文档上说这样可以加快计算速度、节省内存
                cv::SVD::FULL_UV // FULL_UV = 把U和VT补充成单位正交方阵
        );

        // 返回 最小奇异值所对应的 右奇异向量
        // 注意前面说的是右奇异值矩阵的最后一列，
        // 但是在这里因为是vt，转置后了，所以是行；由于A有9列数据，故最后一列的下标为8
        // vt是9-by-9的矩阵 取了最后一行的 1-by-9个元素，然后重排成3-by-3的矩阵输出
        return vt.row(8).reshape(
            0, //转换后的通道数，这里设置为0表示是与前面相同
            3  //转换后的行数,对应V的最后一列 重排成3-by-3的矩阵输出
        );
    }
        ...
        const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
    {
        ...
        // 转换成基础矩阵的形式
        // vt是9-by-9 取出最后一行作为解（也就是9个元素） 然后重排成3-by-3的矩阵
        cv::Mat Fpre = vt.row(8).reshape(0, 3); // v的最后一列

        //基础矩阵的秩为2,而我们不敢保证计算得到的这个结果的秩为2,
        // 所以需要通过第二次奇异值分解,来强制使其秩为2
     ...
     *
     * @param[in] H21            从参考帧1到当前帧的单应矩阵
     * @param[in] H12            从当前帧2到参考帧的单应矩阵
     * @param[in & out] vbMatchesInliers  匹配好的特征点对的Inliers标记
    ...
    float Initializer::CheckHomography(
        ...
        vector<bool> &vbMatchesInliers, //输出：匹配好的特征点对的Inliers标记
        float sigma)                    //估计误差
    {
        // 算法目标： 检查单应变换矩阵
        // 检查方式： 通过H矩阵，进行参考帧和当前帧之间的双向投影，并计算起加权最小二乘投影误差

        // 说明：在已值n维观测数据误差服从 N(0，sigma）的高斯分布时
        // 其误差加权最小二乘结果为  sum_error = SUM(e(i)^T * Q^(-1) * e(i))
        // 其中： e(i) = [e_x,e_y,...]^T,
        //     Q维观测数据协方差矩阵，即sigma * sigma组成的协方差矩阵
        ...
        // 基于卡方检验计算出的阈值（查表可得）（假设测量有一个像素的偏差）
        ...
        for (int i = 0; i < N; i++) // 遍历特征点对
        {
            ...
            // 有 图像2 以及 图像1到2的对应关系H12 就能得到 预测的图像1中的对应坐标（2-dof）
            // 同时图像1中的对应坐标已知（特征匹配） 就能判定 H21的预测叻不叻
            // x1 = H12*x2
            ...
            // 计算投影归一化坐标
            // 最后一行的尺度 1 = (h31inv * u2 + h32inv * v2 + h33inv) = w2in1inv
            const float w2in1inv = 1.0 / (h31inv * u2 + h32inv * v2 + h33inv);
            const float u2in1 = (h11inv * u2 + h12inv * v2 + h13inv) * w2in1inv;
            const float v2in1 = (h21inv * u2 + h22inv * v2 + h23inv) * w2in1inv;

            // 计算重投影误差 = ||p1(i) - H12 * p2(i)||2 // 真实值-预测值 的二范数
            const float squareDist1 = (u1 - u2in1) * (u1 - u2in1) + (v1 - v2in1) * (v1 - v2in1);
            const float chiSquare1 = squareDist1 * invSigmaSquare; // 信息矩阵（协方差）加权
            ...
            // Step 2.3 计算从img1 到 img2 的投影变换误差
            // x1in2 = H21*x1
            // 将图像2中的特征点通过单应变换投影到图像1中
     ...
     * @param[in & out] vbMatchesInliers   匹配的特征点对属于inliers的标记
     * @param[in] sigma            方差，默认为1
     *
     * @return float              返回得分
     */
        ...
        float sigma)                    //方差
    {
        ...
        // 则： p2 一定存在于极线 l2 上，即 p2*l2 = 0.
        // 而  l2 = F*p1 = (a, b, c)^T
        //  line (a, b, c)
        //  point (x, y, 1)
        //  point in line: ax + by + c = 0
        ...
        for (int i = 0; i < N; i++) // 遍历匹配的点对
        {
            ...
            // 已知img1 点对的匹配关系（特征匹配） 预测：img1上的点在img2上的活动范围（极线）
            // 预测结果：极线 （参数 a2 b2 c2）
            // Q: 为什么不是3-dof
            // A: 极线l2上的p2已经确定，不确定的只有斜率
            const float a2 = f11 * u1 + f12 * v1 + f13;
            const float b2 = f21 * u1 + f22 * v1 + f23;
            const float c2 = f31 * u1 + f32 * v1 + f33;

            // Step 2.3 计算误差 e = (a * p2.x + b * p2.y + c) / sqrt(a * a + b * b)
            // 误差： 已知的img2上的点(u2,v2) 到 预测的极线(a2,b2,c2) 的距离
            // 理想情况：img1上的点对应的img2上的点（特征匹配的，点在img2上）就在极线上，所以e=0
            ...
            // ? 可能是为了和CheckHomography 得分统一？ 竹曼觉得可以考虑工程近似
            if (chiSquare1 > th)
                bIn = false;
            else
                // 误差越大，得分越低
            ...
            // 计算 img2上的点 在img1上投影得到的极线 l1= p2 * F21 = (a1,b1,c1)
     ...
     * @details F分解出E，E有四组解，
     *      选择计算的有效三维点（在摄像头前方、投影误差小于阈值、视差角大于阈值）最多的作为最优的解
     *
     * @param[in] vbMatchesInliers      匹配好的特征点对的Inliers标记
     * @param[in] F21            从参考帧1到当前帧2的基础矩阵
     * @param[in] K             相机的内参数矩阵
     ...
     * @param[in] minTriangulated      最小三角化点数量 //? 达不到认为失败
     *
     * @return true             成功初始化
     * @return false             初始化失败
     */
    bool Initializer::ReconstructF(
        ...
        int minTriangulated)
    {
        // Step 1 统计有效（inliers）匹配点个数，并用 N 表示
        ...
        // Step 2 根据 基础矩阵E 和 相机的内参数矩阵K 计算 本质矩阵F
        ...
        // 原理：若某一组合使恢复得到的3D点位于相机正前方的数量最多，
        //    那么该组合就是最佳组合
        // 实现：根据计算的解组合成为四种情况,
        //    并依次调用 Initializer::CheckRT() 进行检查,
        //    得到可以进行三角化测量的点的数目
        ...
        // Step 4.1 使用同样的匹配点分别检查四组解，
        // 记录当前计算的3D点在摄像头前方且投影误差小于阈值的个数，记为有效3D点个数
        int nGood1 = CheckRT(
            R1, t1,                        //当前组解
        ...
        // 在 N=0.9倍的有效（inliers）匹配点个数 和 指定值minTriangulated=50 中取最大的，也就是说至少50个
        ...
        // 如果有最优解 但是 不满足对应的parallax>minParallax，那么返回false表示求解失败
        return false;
    }

    /**
     * @brief  用H矩阵恢复R, t和三维点
     * @details H矩阵分解常见有两种方法：Faugeras SVD-based decomposition 和 Zhang SVD-based decomposition
     *
    ...
    {
        // 目的 ：通过 单应矩阵H 恢复两帧图像之间的 旋转矩阵R 和 平移向量T
        ...
        // 流程:
        //      1. 根据 H矩阵的 奇异值d'=d2 或者 d'=-d2 分别计算 H矩阵分解的 8组解
        //        1.1 讨论 d' > 0 时的 4组解
        //        1.2 讨论 d' < 0 时的 4组解
        //      2. 对 8组解 进行验证，并选择产生相机前方最多3D点的解为最优解
        ...
        // Instead of applying the visibility constraints proposed in the WFaugeras' paper
        ...
        // Step 3 选择最优解：满足下面的四个条件
        // 1. good点数最优解明显大于次优解，这里取 0.75经验值
        // 2. 视角差大于规定的阈值
     ...
     *
     * @param[in] kp1       特征点, in reference frame 1
     * @param[in] kp2       特征点, in current frame 2
     * @param[in] P1        投影矩阵P1
     * @param[in] P2        投影矩阵P2
    ...
    )
    {
        ...
        // 它们都属于 x = aPX模型
        //
        // |x|    |p1 p2  p3  p4 | |X|     |x|    |--p0--||.|
        // |y| = a |p5 p6  p7  p8 | |Y| ===>  |y| = a |--p1--||X|
        // |1|    |p9 p10 p11  p12| |Z|     |z|    |--p2--||.|
        //                  |1|
        // 采用DLT的方法：x叉乘PX = 0
        // |yp2 -  p1|     |0|
        // |p0 -  xp2| X =  |0|
        ...
        //奇异值分解的结果
        cv::Mat u, w, vt;
        //对系数矩阵A进行奇异值分解 A = u * w * vt
        cv::SVD::compute(A, w, u, vt, cv::SVD::MODIFY_A | cv::SVD::FULL_UV);

        //根据前面的结论，奇异值分解右矩阵的最后一行其实就是解，
        // 原理类似于前面的求最小二乘解，四个未知数四个方程正好正定
        //别忘了我们更习惯用列向量来表示一个点的空间坐标
        x3D = vt.row(3).t();
        //为了符合其次坐标的形式，使最后一维为1 [X Y Z 1]^t
        x3D = x3D.rowRange(0, 3) / x3D.at<float>(3);
    }

    /**
     * @brief  归一化特征点到同一尺度，作为后续 normalize DLT的输入
     * @details [x' y' 1]' = T * [x y 1]'
     *      归一化后x', y'的均值为0，且sum(abs(x_i'-0))=1，sum(abs((y_i'-0))=1
     *
     *  为什么要归一化？
     ...
     *
     *  Step 1 计算特征点X,Y坐标的 均值
     *  Step 2 计算特征点X,Y坐标 离均值的平均偏离程度
     ...
     * @param[in] R            旋转矩阵R
     * @param[in] t            平移矩阵t
     * @param[in] vKeys1         参考帧1特征点
     * @param[in] vKeys2         当前帧2特征点
    ...
    int Initializer::CheckRT(
        ...
        float &parallax)
    {
        // 对给出的 特征点对及其R,t , 通过三角化检查解的有效性，也称为 cheirality check
        ...
        for (size_t i = 0, iend = vMatches12.size(); i < iend; i++)
        {
            ...
            //存储这个三角化测量后的3D点在世界坐标系下的坐标 索引号是参考帧关键点的索引号
    ...
    /**
     * @brief  分解 Essential矩阵 得到 R,t
     * @details 分解 E矩阵将得到4组解，这4组解分别为[R1,t],[R1,-t],[R2,t],[R2,-t]
     *
